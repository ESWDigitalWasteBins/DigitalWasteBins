...
class CaptionedImage(Frame):
    ...
    def __init__(self, screen: pygame.display, parent: Frame,
        ...
        Attributes:
            ...
            [ATTR2]: [DESCRIPTION]
        """
        Frame.__init__(self, screen, parent, padx=padx,
                       pady=pady, bg_color=bg_color)
        image_weight = 3
        text_weight = 1
        ...
        self._image = images.scale_image(
            images.load_image(image_path), max_image_size)
        self._image_rect = self._image.get_rect(
            midtop=(self.get_center()[0], self.get_y()))
        self._text_f = Frame(screen, parent=None, x=self.get_x(), y=self._image_rect.bottom,
                             width=self.get_width(), height=round(self.get_height() * text_ratio))
        self._text_frame = TextFrame(
            screen, self._text_f, text, font_file, text_color, text_padx, text_pady)
...
if __name__ == '__main__':
    ...
    ci = CaptionedImage(screen, f, str(
        Path('./assets/img/compost/napkin.png')))
